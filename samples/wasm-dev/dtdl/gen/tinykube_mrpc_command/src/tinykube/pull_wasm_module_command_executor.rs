...
pub type PullWasmModuleRequest =
    CommandRequest<PullWasmModuleRequestPayload, PullWasmModuleResponsePayload>;
pub type PullWasmModuleResponse = CommandResponse<PullWasmModuleResponsePayload>;
pub type PullWasmModuleResponseBuilderError = CommandResponseBuilderError;

/// Builder for [`PullWasmModuleResponse`]
#[derive(Default)]
pub struct PullWasmModuleResponseBuilder {
    inner_builder: CommandResponseBuilder<PullWasmModuleResponsePayload>,
}

impl PullWasmModuleResponseBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: PullWasmModuleResponsePayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `PullWasmModuleResponse`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<PullWasmModuleResponse, PullWasmModuleResponseBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Executor for `pullWasmModule`
pub struct PullWasmModuleCommandExecutor<C>(
    CommandExecutor<PullWasmModuleRequestPayload, PullWasmModuleResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> PullWasmModuleCommandExecutor<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`PullWasmModuleCommandExecutor`]
    ///
    /// # Panics
    ...
    /// Receive the next [`PullWasmModuleRequest`] or [`None`] if there will be no more requests
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure receiving a request
    pub async fn recv(&mut self) -> Option<Result<PullWasmModuleRequest, AIOProtocolError>> {
        self.0.recv().await
    }

    /// Shutdown the [`PullWasmModuleCommandExecutor`]. Unsubscribes from the request topic.
    ///
