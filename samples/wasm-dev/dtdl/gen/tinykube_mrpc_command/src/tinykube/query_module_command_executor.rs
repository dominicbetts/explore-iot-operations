...
pub type QueryModuleRequest = CommandRequest<QueryModuleRequestPayload, QueryModuleResponsePayload>;
pub type QueryModuleResponse = CommandResponse<QueryModuleResponsePayload>;
pub type QueryModuleResponseBuilderError = CommandResponseBuilderError;

/// Builder for [`QueryModuleResponse`]
#[derive(Default)]
pub struct QueryModuleResponseBuilder {
    inner_builder: CommandResponseBuilder<QueryModuleResponsePayload>,
}

impl QueryModuleResponseBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: QueryModuleResponsePayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `QueryModuleResponse`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<QueryModuleResponse, QueryModuleResponseBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Executor for `queryModule`
pub struct QueryModuleCommandExecutor<C>(
    CommandExecutor<QueryModuleRequestPayload, QueryModuleResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> QueryModuleCommandExecutor<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`QueryModuleCommandExecutor`]
    ///
    /// # Panics
    ...
    /// Receive the next [`QueryModuleRequest`] or [`None`] if there will be no more requests
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure receiving a request
    pub async fn recv(&mut self) -> Option<Result<QueryModuleRequest, AIOProtocolError>> {
        self.0.recv().await
    }

    /// Shutdown the [`QueryModuleCommandExecutor`]. Unsubscribes from the request topic.
    ///
