...
pub type StartModuleRequest = CommandRequest<StartModuleRequestPayload, StartModuleResponsePayload>;
pub type StartModuleResponse = CommandResponse<StartModuleResponsePayload>;
pub type StartModuleResponseBuilderError = CommandResponseBuilderError;

/// Builder for [`StartModuleResponse`]
#[derive(Default)]
pub struct StartModuleResponseBuilder {
    inner_builder: CommandResponseBuilder<StartModuleResponsePayload>,
}

impl StartModuleResponseBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: StartModuleResponsePayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `StartModuleResponse`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<StartModuleResponse, StartModuleResponseBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Executor for `startModule`
pub struct StartModuleCommandExecutor<C>(
    CommandExecutor<StartModuleRequestPayload, StartModuleResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> StartModuleCommandExecutor<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`StartModuleCommandExecutor`]
    ///
    /// # Panics
    ...
    /// Receive the next [`StartModuleRequest`] or [`None`] if there will be no more requests
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure receiving a request
    pub async fn recv(&mut self) -> Option<Result<StartModuleRequest, AIOProtocolError>> {
        self.0.recv().await
    }

    /// Shutdown the [`StartModuleCommandExecutor`]. Unsubscribes from the request topic.
    ///
