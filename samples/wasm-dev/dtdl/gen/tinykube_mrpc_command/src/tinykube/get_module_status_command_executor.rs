...
pub type GetModuleStatusRequest =
    CommandRequest<GetModuleStatusRequestPayload, GetModuleStatusResponsePayload>;
pub type GetModuleStatusResponse = CommandResponse<GetModuleStatusResponsePayload>;
pub type GetModuleStatusResponseBuilderError = CommandResponseBuilderError;

/// Builder for [`GetModuleStatusResponse`]
#[derive(Default)]
pub struct GetModuleStatusResponseBuilder {
    inner_builder: CommandResponseBuilder<GetModuleStatusResponsePayload>,
}

impl GetModuleStatusResponseBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: GetModuleStatusResponsePayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `GetModuleStatusResponse`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<GetModuleStatusResponse, GetModuleStatusResponseBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Executor for `getModuleStatus`
pub struct GetModuleStatusCommandExecutor<C>(
    CommandExecutor<GetModuleStatusRequestPayload, GetModuleStatusResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> GetModuleStatusCommandExecutor<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`GetModuleStatusCommandExecutor`]
    ///
    /// # Panics
    ...
    /// Receive the next [`GetModuleStatusRequest`] or [`None`] if there will be no more requests
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure receiving a request
    pub async fn recv(&mut self) -> Option<Result<GetModuleStatusRequest, AIOProtocolError>> {
        self.0.recv().await
    }

    /// Shutdown the [`GetModuleStatusCommandExecutor`]. Unsubscribes from the request topic.
    ///
