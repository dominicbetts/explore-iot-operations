...
use super::get_module_status_request_payload::GetModuleStatusRequestPayload;
use super::get_module_status_response_payload::GetModuleStatusResponsePayload;
use super::MODEL_ID;
use super::REQUEST_TOPIC_PATTERN;

pub type GetModuleStatusRequest = CommandRequest<GetModuleStatusRequestPayload>;
pub type GetModuleStatusResponse = CommandResponse<GetModuleStatusResponsePayload>;
pub type GetModuleStatusRequestBuilderError = CommandRequestBuilderError;

#[derive(Default)]
/// Builder for [`GetModuleStatusRequest`]
pub struct GetModuleStatusRequestBuilder {
    inner_builder: CommandRequestBuilder<GetModuleStatusRequestPayload>,
    set_executor_id: bool,
}

impl GetModuleStatusRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: GetModuleStatusRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `GetModuleStatusRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<GetModuleStatusRequest, GetModuleStatusRequestBuilderError> {
        if !self.set_executor_id {
            return Err(GetModuleStatusRequestBuilderError::UninitializedField(
                "executor_id",
            ));
        }
...
/// Command Invoker for `getModuleStatus`
pub struct GetModuleStatusCommandInvoker<C>(
    CommandInvoker<GetModuleStatusRequestPayload, GetModuleStatusResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> GetModuleStatusCommandInvoker<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`GetModuleStatusCommandInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        topic_token_map.insert("commandName".to_string(), "getModuleStatus".to_string());

        let invoker_options = invoker_options_builder
            .request_topic_pattern(REQUEST_TOPIC_PATTERN)
            .command_name("getModuleStatus")
    ...
    /// Invokes the [`GetModuleStatusRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<GetModuleStatusResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`GetModuleStatusCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
