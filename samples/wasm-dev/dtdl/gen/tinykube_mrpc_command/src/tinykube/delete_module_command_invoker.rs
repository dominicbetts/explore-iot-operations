...
use super::delete_module_request_payload::DeleteModuleRequestPayload;
use super::delete_module_response_payload::DeleteModuleResponsePayload;
use super::MODEL_ID;
use super::REQUEST_TOPIC_PATTERN;

pub type DeleteModuleRequest = CommandRequest<DeleteModuleRequestPayload>;
pub type DeleteModuleResponse = CommandResponse<DeleteModuleResponsePayload>;
pub type DeleteModuleRequestBuilderError = CommandRequestBuilderError;

#[derive(Default)]
/// Builder for [`DeleteModuleRequest`]
pub struct DeleteModuleRequestBuilder {
    inner_builder: CommandRequestBuilder<DeleteModuleRequestPayload>,
    set_executor_id: bool,
}

impl DeleteModuleRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: DeleteModuleRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `DeleteModuleRequest`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<DeleteModuleRequest, DeleteModuleRequestBuilderError> {
        if !self.set_executor_id {
            return Err(DeleteModuleRequestBuilderError::UninitializedField(
                "executor_id",
            ));
        }
...
/// Command Invoker for `deleteModule`
pub struct DeleteModuleCommandInvoker<C>(
    CommandInvoker<DeleteModuleRequestPayload, DeleteModuleResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> DeleteModuleCommandInvoker<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`DeleteModuleCommandInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        topic_token_map.insert("commandName".to_string(), "deleteModule".to_string());

        let invoker_options = invoker_options_builder
            .request_topic_pattern(REQUEST_TOPIC_PATTERN)
            .command_name("deleteModule")
    ...
    /// Invokes the [`DeleteModuleRequest`]
    ///
    /// # Errors
    ...
    ) -> Result<DeleteModuleResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`DeleteModuleCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
