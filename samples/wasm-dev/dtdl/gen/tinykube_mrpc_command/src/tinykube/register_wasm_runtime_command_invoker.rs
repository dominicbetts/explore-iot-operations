...
pub type RegisterWasmRuntimeRequest = CommandRequest<RegisterWasmRuntimeRequestPayload>;
pub type RegisterWasmRuntimeResponse = CommandResponse<RegisterWasmRuntimeResponsePayload>;
pub type RegisterWasmRuntimeRequestBuilderError = CommandRequestBuilderError;

#[derive(Default)]
/// Builder for [`RegisterWasmRuntimeRequest`]
pub struct RegisterWasmRuntimeRequestBuilder {
    inner_builder: CommandRequestBuilder<RegisterWasmRuntimeRequestPayload>,
    set_executor_id: bool,
}

impl RegisterWasmRuntimeRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: RegisterWasmRuntimeRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `RegisterWasmRuntimeRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<RegisterWasmRuntimeRequest, RegisterWasmRuntimeRequestBuilderError> {
        if !self.set_executor_id {
            return Err(RegisterWasmRuntimeRequestBuilderError::UninitializedField(
                "executor_id",
            ));
        }
...
/// Command Invoker for `registerWasmRuntime`
pub struct RegisterWasmRuntimeCommandInvoker<C>(
    CommandInvoker<RegisterWasmRuntimeRequestPayload, RegisterWasmRuntimeResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> RegisterWasmRuntimeCommandInvoker<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`RegisterWasmRuntimeCommandInvoker`]
    ///
    /// # Panics
    ...
    /// Invokes the [`RegisterWasmRuntimeRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: RegisterWasmRuntimeRequest,
    ) -> Result<RegisterWasmRuntimeResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`RegisterWasmRuntimeCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
