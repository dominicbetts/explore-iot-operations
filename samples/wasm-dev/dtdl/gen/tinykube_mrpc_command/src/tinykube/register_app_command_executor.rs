...
pub type RegisterAppRequest = CommandRequest<RegisterAppRequestPayload, RegisterAppResponsePayload>;
pub type RegisterAppResponse = CommandResponse<RegisterAppResponsePayload>;
pub type RegisterAppResponseBuilderError = CommandResponseBuilderError;

/// Builder for [`RegisterAppResponse`]
#[derive(Default)]
pub struct RegisterAppResponseBuilder {
    inner_builder: CommandResponseBuilder<RegisterAppResponsePayload>,
}

impl RegisterAppResponseBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: RegisterAppResponsePayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `RegisterAppResponse`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<RegisterAppResponse, RegisterAppResponseBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Executor for `registerApp`
pub struct RegisterAppCommandExecutor<C>(
    CommandExecutor<RegisterAppRequestPayload, RegisterAppResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> RegisterAppCommandExecutor<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`RegisterAppCommandExecutor`]
    ///
    /// # Panics
    ...
    /// Receive the next [`RegisterAppRequest`] or [`None`] if there will be no more requests
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure receiving a request
    pub async fn recv(&mut self) -> Option<Result<RegisterAppRequest, AIOProtocolError>> {
        self.0.recv().await
    }

    /// Shutdown the [`RegisterAppCommandExecutor`]. Unsubscribes from the request topic.
    ///
