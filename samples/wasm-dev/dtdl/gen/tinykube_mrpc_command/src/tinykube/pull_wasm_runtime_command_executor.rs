...
pub type PullWasmRuntimeRequest =
    CommandRequest<PullWasmRuntimeRequestPayload, PullWasmRuntimeResponsePayload>;
pub type PullWasmRuntimeResponse = CommandResponse<PullWasmRuntimeResponsePayload>;
pub type PullWasmRuntimeResponseBuilderError = CommandResponseBuilderError;

/// Builder for [`PullWasmRuntimeResponse`]
#[derive(Default)]
pub struct PullWasmRuntimeResponseBuilder {
    inner_builder: CommandResponseBuilder<PullWasmRuntimeResponsePayload>,
}

impl PullWasmRuntimeResponseBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: PullWasmRuntimeResponsePayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `PullWasmRuntimeResponse`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<PullWasmRuntimeResponse, PullWasmRuntimeResponseBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Executor for `pullWasmRuntime`
pub struct PullWasmRuntimeCommandExecutor<C>(
    CommandExecutor<PullWasmRuntimeRequestPayload, PullWasmRuntimeResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> PullWasmRuntimeCommandExecutor<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`PullWasmRuntimeCommandExecutor`]
    ///
    /// # Panics
    ...
    /// Receive the next [`PullWasmRuntimeRequest`] or [`None`] if there will be no more requests
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure receiving a request
    pub async fn recv(&mut self) -> Option<Result<PullWasmRuntimeRequest, AIOProtocolError>> {
        self.0.recv().await
    }

    /// Shutdown the [`PullWasmRuntimeCommandExecutor`]. Unsubscribes from the request topic.
    ///
