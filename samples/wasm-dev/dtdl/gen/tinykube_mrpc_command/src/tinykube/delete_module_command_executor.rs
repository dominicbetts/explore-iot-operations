...
pub type DeleteModuleRequest =
    CommandRequest<DeleteModuleRequestPayload, DeleteModuleResponsePayload>;
pub type DeleteModuleResponse = CommandResponse<DeleteModuleResponsePayload>;
pub type DeleteModuleResponseBuilderError = CommandResponseBuilderError;

/// Builder for [`DeleteModuleResponse`]
#[derive(Default)]
pub struct DeleteModuleResponseBuilder {
    inner_builder: CommandResponseBuilder<DeleteModuleResponsePayload>,
}

impl DeleteModuleResponseBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: DeleteModuleResponsePayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `DeleteModuleResponse`
    ///
    /// # Errors
    /// If a required field has not been initialized
    #[allow(clippy::missing_panics_doc)] // The panic is not possible
    pub fn build(&mut self) -> Result<DeleteModuleResponse, DeleteModuleResponseBuilderError> {
        self.inner_builder.build()
    }
}

/// Command Executor for `deleteModule`
pub struct DeleteModuleCommandExecutor<C>(
    CommandExecutor<DeleteModuleRequestPayload, DeleteModuleResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> DeleteModuleCommandExecutor<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`DeleteModuleCommandExecutor`]
    ///
    /// # Panics
    ...
    /// Receive the next [`DeleteModuleRequest`] or [`None`] if there will be no more requests
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure receiving a request
    pub async fn recv(&mut self) -> Option<Result<DeleteModuleRequest, AIOProtocolError>> {
        self.0.recv().await
    }

    /// Shutdown the [`DeleteModuleCommandExecutor`]. Unsubscribes from the request topic.
    ///
