...
use super::notify_module_update_request_payload::NotifyModuleUpdateRequestPayload;
use super::notify_module_update_response_payload::NotifyModuleUpdateResponsePayload;
use super::MODEL_ID;
use super::REQUEST_TOPIC_PATTERN;

pub type NotifyModuleUpdateRequest = CommandRequest<NotifyModuleUpdateRequestPayload>;
pub type NotifyModuleUpdateResponse = CommandResponse<NotifyModuleUpdateResponsePayload>;
pub type NotifyModuleUpdateRequestBuilderError = CommandRequestBuilderError;

#[derive(Default)]
/// Builder for [`NotifyModuleUpdateRequest`]
pub struct NotifyModuleUpdateRequestBuilder {
    inner_builder: CommandRequestBuilder<NotifyModuleUpdateRequestPayload>,
    set_executor_id: bool,
}

impl NotifyModuleUpdateRequestBuilder {
    ...
    pub fn payload(
        &mut self,
        payload: NotifyModuleUpdateRequestPayload,
    ) -> Result<&mut Self, AIOProtocolError> {
        self.inner_builder.payload(payload)?;
        Ok(self)
    }

    /// Builds a new `NotifyModuleUpdateRequest`
    ///
    /// # Errors
    ...
    pub fn build(
        &mut self,
    ) -> Result<NotifyModuleUpdateRequest, NotifyModuleUpdateRequestBuilderError> {
        if !self.set_executor_id {
            return Err(NotifyModuleUpdateRequestBuilderError::UninitializedField(
                "executor_id",
            ));
        }
...
/// Command Invoker for `notifyModuleUpdate`
pub struct NotifyModuleUpdateCommandInvoker<C>(
    CommandInvoker<NotifyModuleUpdateRequestPayload, NotifyModuleUpdateResponsePayload, C>,
)
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static;

impl<C> NotifyModuleUpdateCommandInvoker<C>
where
    C: ManagedClient + Clone + Send + Sync + 'static,
    C::PubReceiver: Send + Sync + 'static,
{
    /// Creates a new [`NotifyModuleUpdateCommandInvoker`]
    ///
    /// # Panics
    ...
    pub fn new(
        ...
        topic_token_map.insert("commandName".to_string(), "notifyModuleUpdate".to_string());

        let invoker_options = invoker_options_builder
            .request_topic_pattern(REQUEST_TOPIC_PATTERN)
            .command_name("notifyModuleUpdate")
    ...
    /// Invokes the [`NotifyModuleUpdateRequest`]
    ///
    /// # Errors
    /// [`AIOProtocolError`] if there is a failure invoking the request
    pub async fn invoke(
        &self,
        request: NotifyModuleUpdateRequest,
    ) -> Result<NotifyModuleUpdateResponse, AIOProtocolError> {
        self.0.invoke(request).await
    }

    /// Shutdown the [`NotifyModuleUpdateCommandInvoker`]. Unsubscribes from the response topic and cancels the receiver loop to drop the receiver and to prevent the task from looping indefinitely.
    ///
